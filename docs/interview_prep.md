...
The short version: I rebuilt a stock-picking backtest that originally tested one strategy on one lucky path with monthly data. I turned it into a walk-forward comparison of six allocation strategies over ten years of daily NSE data, with real transaction costs, statistical significance testing, and a volatility-forecasting model I benchmarked against GARCH. The most useful thing I found wasn't "my strategy wins" — it was that of six strategies tested against Nifty, only plain equal-weight held up statistically, and a genuinely better volatility forecast didn't actually translate into a better portfolio. I'd rather show up with a project that found that than one claiming some inflated CAGR number.

## "What's your Sharpe ratio / CAGR?"

I'd point to the comparison table instead of quoting one number. Equal-weight came out highest at 26.4% CAGR and 0.96 Sharpe vs. Nifty's 12.4% and 0.39 — and the more honest answer is in `results/tables/significance_tests.csv`: it's also the *only* strategy with a bootstrap 95% confidence interval that actually excludes zero versus Nifty. Everything else — including the fancier optimizers like max-Sharpe and min-variance — is directionally positive but I can't say it's distinguishable from luck over this sample. The fact that the simplest strategy is the one that held up isn't a disappointing result, it's actually a well-documented finding (DeMiguel, Garlappi & Uppal, 2009): with more assets for an optimizer to estimate expected returns and correlations for, there's more room for that estimation error to get exploited. I'd rather explain that than round up to "my optimizer wins."
